...
def queryLastData():
	...
	for row in curs.execute("SELECT * FROM GPSDATA"):
		...
		longitude = row[1]
	conn.close()
	return time, temp, pres, alt, latitude, longitude
...
def queryHistData(nbSamples):
	...
	curs.execute("SELECT * FROM GPSDATA ORDER BY timestamp DESC LIMIT " + str(nbSamples))
	data = curs.fetchall()
	for row in reversed(data):
		lng.append(row[0])
		lat.append(row[1])
	conn.close()
	return dates, temps, press, alts, lng, lat
...
def index():
	...
	time, temp, pres, alt, lat, lon = queryLastData()
	templateData = {
		'time': time,
		'temperature': temp,
		'pressure': pres,
