...
void keyPressed() {
  if ((keyPressed == true) && (key == CODED)) {
    if (keyCode == UP && validFutLoc(1)){// forward
     locy = locy - movSpeed*cos(heading);
     locx = locx + movSpeed*sin(heading);
    } else if (keyCode == DOWN && validFutLoc(0)) {//backwads
     locy = locy + movSpeed*cos(heading);
     locx = locx - movSpeed*sin(heading);
...
boolean validFutLoc(int fw){
  PVector futLoc;
  if(fw == 0){
  futLoc = new PVector(locx - 2*movSpeed*sin(heading) ,locy + 2*movSpeed*cos(heading));
  } else {
    futLoc = new PVector(locx + 2*movSpeed*sin(heading) ,locy - 2*movSpeed*cos(heading));
  }
